...
export class DescribeTraceHandler implements ICommandHandler<DescribeTraceCommand> {
  constructor(
  ...
  async execute(cmd: DescribeTraceCommand): Promise<void> {
    ...
    trace.describe({
      ...
      revelationTechnique: cmd.revelationTechnique,
    });

    await this.repo.save(trace, {
      eventType: AuditEventTypeEnum.TRACE_DESCRIBED,
